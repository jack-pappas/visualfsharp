...
let ResolveCompletionsInType (ncenv: NameResolver) nenv (completionTargets: ResolveCompletionTargets) m ad statics typ =
    ...
    let suppressedMethNames = Zset.ofList String.order (Seq.append pinfoMethNames einfoMethNames |> Seq.toList)
...
let rec ResolvePartialLongIdentInModuleOrNamespace (ncenv: NameResolver) nenv isApplicableMeth m ad (modref:ModuleOrNamespaceRef) plid allowObsolete =
    ...
    let tycons = 
        ...
        |> Seq.filter (fun tycon -> not (IsTyconUnseen ad g ncenv.amap m (modref.MkNestedTyconRef tycon)))
        |> Seq.toList
    ...
    | [] ->
       ...
       @ (mty.ExceptionDefinitionsByDemangledName 
          |> NameMap.range 
          |> Seq.map modref.MkNestedTyconRef
    ...
    |  [] -> 
       ...
       let activePatternItems = 
           ...
           | OpenQualified ->
               nenv.ePatItems
               |> NameMap.range
               |> Seq.filter (function Item.ActivePatternCase _ -> true | _ -> false)
